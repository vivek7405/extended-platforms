...
import { UserProps } from "@/modules/people/types";
...
import { getInvites } from "@/modules/people/actions/get-invites";
...
export default function People() {
  ...
  const { data: teammates, isLoading: isLoadingTeammates } = useSWR(
    "teammates",
    () => getTeammates(id),
  );
  const { data: invites, isLoading: isLoadingInvites } = useSWR("invites", () =>
    getInvites(id),
  );

  const users = currentTab === "Members" ? teammates : invites;
  const isLoading =
    currentTab === "Members" ? isLoadingTeammates : isLoadingInvites;
            ...
            users.length > 0 ? (
              users.map((user) => (
                <UserCard
                  ...
                  isOwner={teammates ? teammates[0].role === "owner" : false}
                />
              ))
            ) : (
