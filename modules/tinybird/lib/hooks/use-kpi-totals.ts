...
function getNotFoundColumnsWarning(error: QueryError | null): string | null {
  if (!error) return null;
  try {
    // parsing the error message to get the columns that are not found
    const rawColumns = error.message
...
export default function useKpiTotals(domain) {
  const { from, to } = useDateFilter();
  // const { warning, ...query } = useQuery(
  //   [from, to, domain, "kpiTotals"],
  //   getKpiTotals,
  // );
  const query = useSWR("kpiTotals", () => getKpiTotals(from, to, domain));
  return {
    ...query,
    status: getStatus(
      query.data,
    ...
    warning: getNotFoundColumnsWarning(query.error),
  };
}
